...
export async function loginMicrosoft(authCode: string): Promise<MicrosoftProfile> {
  ...
  ).then(r => r.json()) as MSTokenResponse

  return msTokenToProfile(msToken.access_token, msToken.refresh_token)
}

export async function refreshMicrosoft(refreshToken: string): Promise<MicrosoftProfile> {
  const msToken = await fetch(
    `https://login.live.com/oauth20_token.srf?client_id=00000000402b5328&refresh_token=${refreshToken}&grant_type=refresh_token&scope=service::user.auth.xboxlive.com::MBI_SSL&redirect_uri=https://login.live.com/oauth20_desktop.srf`
  ).then(r => r.json()) as MSTokenResponse
